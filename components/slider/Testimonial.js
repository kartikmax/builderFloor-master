...
import { useMedia } from "../../functions/use-media";

SwiperCore.use([Autoplay, Navigation]);
const TestimonialSlider = () => {
  const isMobile = useMedia("(max-width: 990px)");
  const slides = isMobile ? 1 : 4;
  const data = [
    {
      avatar: "1.png",
      name: "Wade Warren",
    ...
    },
    {
      ...
      company: "Starbucks",
      quote:
        "Their bloods alcohol levels rose to 0.007 to 0.02 o/oo (parts per thousand), or 0.7 to 2.0 mg/L",
      bg: "bd-bg-10",
    },
    {
      avatar: "4.png",
              ...
              <SwiperSlide key={i}>
                <div className="swiper-slide active">
                  <div className={`card-grid-style-3 hover-up ${item.bg}`}>
                    ...
                    <p className="text-body-small text-desc color-gray-500 mt-20">
                      {item.quote}
                    </p>
                  </div>
                </div>
